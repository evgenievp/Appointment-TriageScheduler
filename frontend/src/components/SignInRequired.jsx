...
export default function SignInRequired({ returnTo, style }) {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const location = useLocation();
  const back = encodeURIComponent(returnTo ?? location.pathname);
  ...
  return (
    <div style={style}>
      <EmptyState
        ...
        action={
          <Button onClick={() => navigate(`/login?from=${back}`)}>
            {t('nav.login')}
          </Button>
        }
        secondaryAction={
          <Button variant="secondary" onClick={() => navigate(`/register?from=${back}`)}>
